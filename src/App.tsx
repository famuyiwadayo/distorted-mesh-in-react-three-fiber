...
import { Icosahedron, MeshDistortMaterial, useCubeTextureLoader, useTextureLoader, Html } from 'drei'

function MainSphere({ material }: any) {
...
function Scene() {
  ...
  const matRef = useResource<any>()

  const material = useMemo(() => matRef.current, [matRef])
  ...
  return (
    <>
      <MeshDistortMaterial
        ...
        distort={0.4}
      />
      {matRef && <Instances material={matRef?.current} />}
    </>
  )
}
...
function App() {
  ...
  const color  = new THREE.Color("#161616")
  return (
    <Canvas
      colorManagement
    ...
    >
      ...
      <fog color={color} attach="fog" near={8} far={30} />
